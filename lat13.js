...
function trainSeating (penumpang) {
  var gerbong=["VVIP","VIP","Regular"]
    var Obj={}
  for (var i = 0; i < gerbong.length; i++) {
      Obj[gerbong[i]]=[]
    for (var j = 0; j < penumpang.length; j++) {
      if (gerbong[i]===penumpang[j].gerbong) {
        Obj[gerbong[i]].push({name : penumpang[j].nama, seat: penumpang[j].seat})
      }
    }
  }
  return Obj
